    ...
    class polygon
    {
        ...
        private ArrayList getYmin(int y)
        {
            ...
            for (int i = 0; i < m_NumVertex; i++)
            {
                ...
                if (i != m_NumVertex - 1)
                {
                    if (polygonVertex[i].Y == polygonVertex[i + 1].Y)
                        continue;
                ...
                {
                    if (polygonVertex[0].Y == polygonVertex[m_NumVertex - 1].Y)
                        continue;
                ...
                //上述代码是判断多边形的顶点y是否有与传入进来y相同的情况
                //满足情况执行下面情况，并在NET中添加一个动态数组，该动态数组包含三个值，Ymax，ΔX，X|Ymin 
        ...
        public void linefill(IntPtr hDC, Color color)
        {
            ...
            for (int i = 0; i < LineNum; i++)
            {
                ...
                //从NET中取出的边列表进行排序  NET是个动态数组,代表所有扫描线的集合,NET[i]代表第i条扫描线得到在Y=i时的边表集合，NET[i][j]才表示单一的边表，也是用动态数组进行表示的
                ...
                {
                    int x1, x2;
                    x1 = (int)(getItemX((ArrayList)AET[j]) + 0.5);
                    x2 = (int)(getItemX((ArrayList)AET[j + 1]) + 0.5);
                    int y1 = i + y;
                ...
                for (int j = 0; j < AET.Count; j++)
                {
                    ...
                    if (getItemYmax((ArrayList)AET[j]) == i + y + 1|| getItemtx((ArrayList)AET[j]) == float.PositiveInfinity)
                    {
                        stack.Push(j);//进栈
                    }
                ...
                while (stack.Count > 0)
                {
                    int tempa = (int)stack.Pop();//出栈
                    AET.RemoveAt(tempa);//根据索引来进行删除
                }
